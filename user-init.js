...
(function () {
  ...
  // ── 7b. Simulate Referral buttons hide karo ──
  function hideSimulateButtons() {
    document.querySelectorAll('button, [onclick]').forEach(el => {
      const txt = el.textContent || '';
      const onclick = el.getAttribute('onclick') || '';
      if (
        txt.includes('Simulate Referral') ||
        onclick.includes('simulateReferral') ||
        onclick.includes('SimulateRef') ||
        txt.includes('Simulate')
      ) {
