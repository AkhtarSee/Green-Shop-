...
(function () {
  ...
  // ── COIN RATES (FIXED: 1 coin = ₹1) ──
  const REFERRAL_NEW_USER = 25;
  const REFERRAL_BONUS    = 50;
  const DAILY_COINS       = 1;

  // ── FREE GAME PRIZES (FIXED) ──
  const FREE_RACE_PRIZES = {
    1: 30,   // 🥇 1st
    2: 20,   // 🥈 2nd
    3: 10    // 🥉 3rd
    // baaki kuch nahi
  ...
  const FREE_TB_PRIZES = {
    1: 30,
    2: 20,
    3: 10
  };

  // ── PAID GAME (FIXED: 🪙10 entry, 1st🪙200) ──
  const PAID_ENTRY = 10;
  const PAID_RACE_PRIZES = {
    1: 200,
    2: 100,
    3: 50
  };

  // ── WIN RACE coins (FIXED: 🪙30) ──
  const WIN_RACE_COINS = 30;
  ...
  function applyUserToApp(user) {
    ...
    if (pwRate) pwRate.textContent = '🔄 1 Coin = ₹1 Grocery Discount';

    const pwInr = document.getElementById('pWalletInr');
    if (pwInr) pwInr.textContent = `≈ ₹${coinsToRs(user.coins||0).toFixed(2)} grocery discount value`;
  ...
  function showRegistration(referredBy) {
    ...
    window._gsRegister = async function () {
      ...
      if (referredBy) await dbGiveReferralBonus(referredBy, REFERRAL_BONUS);
